                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="float-right mb-3">
                                <a href="{{ route('award.create') }}" class="btn btn-success">Add Award</a>
                            </div>
                            <!-- /.card-header -->

                            <table id="award_table" class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        <th>Sr.No.</th>
                                        <th>Title</th>
                                    ...
                                    @if(count($data) > 0)
                                        ...
                                        @foreach ($data as $row)
                                            <tr>
                                                ...
                                                <form action="{{ route('award.destroy', $row->id) }}" method="POST">
                                                    <td>
                                                        <a class="btn btn-primary"
                                                            href="{{ route('award.edit', $row->id) }}">Edit</a>
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-danger"
                                                            onclick="return confirm('Are you sure want to delete this award?')">Delete</button>
                                                    </td>
                                                </form>
                                            </tr>
                                        @endforeach
